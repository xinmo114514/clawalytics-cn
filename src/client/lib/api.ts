...
export type DesktopNotificationTrigger =
  DesktopPreferences['notificationTrigger']
...
): Promise<EnhancedSessionsResponse> {
...
export async function getAgentDailyCosts(
  ...
  const { data } = await api.get<AgentDailyCost[]>(`/agents/${agentId}/daily`, {
    params: { days },
  })
  return data
}
...
export async function getAlerts(
  ...
  const { data } = await api.get<SecurityAlert[]>('/security/alerts', {
    params,
  })
  return data
}
...
export async function getUnacknowledgedAlerts(): Promise<SecurityAlert[]> {
  const { data } = await api.get<SecurityAlert[]>(
...
export async function getConnectionEvents(
  ...
  const { data } = await api.get<ConnectionEvent[]>('/security/connections', {
    params,
  })
  return data
}
...
export async function getRecentConnections(
  ...
  const { data } = await api.get<ConnectionEvent[]>(
    '/security/connections/recent',
    {
      params,
    }
  )
...
export async function getOutboundCalls(
  ...
  const { data } = await api.get<OutboundCallsResponse>('/tools', {
    params: filters,
  })
  return data
}
...
export async function getAuditLog(
  ...
  const { data } = await api.get<AuditLogResponse>('/audit', {
    params: filters,
  })
  return data
}
...
export async function getModelStats(days = 30): Promise<ModelStats> {
  const { data } = await api.get<ModelStats>('/models/stats', {
    params: { days },
  })
  return data
}
...
export async function getModelDailyUsage(
  days = 30
): Promise<ModelDailyUsage[]> {
  const { data } = await api.get<ModelDailyUsage[]>('/models/daily', {
    params: { days },
  })
  return data
}
...
export async function getProviderSummary(
  ...
  const { data } = await api.get<ProviderSummary[]>('/models/providers', {
    params: { days },
  })
  return data
}
...
export async function getCustomRates(): Promise<
  Record<string, Record<string, CustomRateEntry>>
> {
...
export async function updateCustomRate(
  ...
  const { data } = await api.put<CustomRateUpdateResult>(
    `/config/custom-rates/${provider}/${model}`,
    rate
  )
  return data
}
