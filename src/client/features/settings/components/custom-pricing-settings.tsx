...
import { getCustomRates, updateCustomRate, deleteCustomRate } from '@/lib/api'
...
function validateRate(value: string): { valid: boolean; message?: string } {
  ...
  if (
    value.includes('.') &&
    value.split('.')[1].length > VALIDATION_RULES.decimalPlaces
  ) {
    return {
      valid: false,
      message: `Max ${VALIDATION_RULES.decimalPlaces} decimal places`,
    }
  }
  return { valid: true }
}
...
export function CustomPricingSettings() {
  ...
  const [recordToDelete, setRecordToDelete] = useState<PricingRecord | null>(
    null
  )
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      const rate = {
        ...
        ...(editForm.cacheRead
          ? { cacheRead: Number.parseFloat(editForm.cacheRead) }
          : {}),
        ...(editForm.cacheWrite
          ? { cacheWrite: Number.parseFloat(editForm.cacheWrite) }
          : {}),
      }

      const result = await updateCustomRate(
        editingRecord.provider,
        editingRecord.model,
        rate
      )
      ...
      if (result.priceChange?.significant) {
        ...
        toast.success(text('定价已更新', 'Pricing updated'))
      }
  ...
  async function handleAddNew() {
    if (!newProvider.trim() || !newModel.trim()) {
      toast.error(
        text('请填写提供商和模型名称', 'Please fill in provider and model name')
      )
      return
    }
    ...
    try {
      ...
      const rate = {
        ...
        ...(newForm.cacheRead
          ? { cacheRead: Number.parseFloat(newForm.cacheRead) }
          : {}),
        ...(newForm.cacheWrite
          ? { cacheWrite: Number.parseFloat(newForm.cacheWrite) }
          : {}),
      }
  ...
  return (
    <>
        ...
        {isLoading ? (
          ...
          <div className='py-8 text-center text-muted-foreground'>
            ...
            <p className='mt-1 text-sm'>
              {text(
                '点击上方按钮添加新的自定义定价',
                'Click the button above to add new custom pricing'
              )}
            </p>
          </div>
        ) : (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className='text-right'>
                    {text('输出价格 (¥/M)', 'Output (¥/M)')}
                  </TableHead>
                  <TableHead className='text-right'>
                    {text('缓存读取 (¥/M)', 'Cache Read (¥/M)')}
                  </TableHead>
                  <TableHead className='text-right'>
                    {text('缓存写入 (¥/M)', 'Cache Write (¥/M)')}
                  </TableHead>
                ...
                {records.map((record) => (
                  <TableRow key={`${record.provider}/${record.model}`}>
                    ...
                    <TableCell className='text-right tabular-nums'>
                      {formatRate(record.input)}
                    </TableCell>
                    <TableCell className='text-right tabular-nums'>
                      {formatRate(record.output)}
                    </TableCell>
                    <TableCell className='text-right tabular-nums'>
                      {formatRate(record.cacheRead)}
                    </TableCell>
                    <TableCell className='text-right tabular-nums'>
                      {formatRate(record.cacheWrite)}
                    </TableCell>
                    <TableCell>
      ...
      <Dialog open={!!editingRecord} onOpenChange={() => closeEditDialog()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{text('编辑定价', 'Edit Pricing')}</DialogTitle>
            <DialogDescription>
              {editingRecord &&
                `${editingRecord.provider} / ${editingRecord.model}`}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className='space-y-4 py-4'>
            ...
            <div className='space-y-2'>
              <label className='text-sm font-medium'>
                {text(
                  '缓存读取价格 (¥/M tokens，可选)',
                  'Cache Read Price (¥/M tokens, optional)'
                )}
              </label>
              <Input
                value={editForm.cacheRead}
                onChange={(e) =>
                  setEditForm({ ...editForm, cacheRead: e.target.value })
                }
                placeholder='0.00'
                type='number'
                step='0.0001'
            ...
            <div className='space-y-2'>
              <label className='text-sm font-medium'>
                {text(
                  '缓存写入价格 (¥/M tokens，可选)',
                  'Cache Write Price (¥/M tokens, optional)'
                )}
              </label>
              <Input
                value={editForm.cacheWrite}
                onChange={(e) =>
                  setEditForm({ ...editForm, cacheWrite: e.target.value })
                }
                placeholder='0.00'
                type='number'
                step='0.0001'
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{text('确认删除', 'Confirm Delete')}</DialogTitle>
            <DialogDescription>
              {text(
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isSaving}
            >
              {isSaving && <RefreshCw className='mr-2 size-4 animate-spin' />}
              {text('删除', 'Delete')}
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={isAddingNew} onOpenChange={() => closeAddDialog()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{text('添加新定价', 'Add New Pricing')}</DialogTitle>
            <DialogDescription>
              {text(
                '为新的提供商/模型添加自定义定价',
                'Add custom pricing for a new provider/model'
              )}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className='space-y-4 py-4'>
            ...
            <div className='space-y-2'>
              ...
              <Input
                value={newForm.input}
                onChange={(e) =>
                  setNewForm({ ...newForm, input: e.target.value })
                }
                placeholder='0.00'
                type='number'
                step='0.0001'
            ...
            <div className='space-y-2'>
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewForm({ ...newForm, output: e.target.value })
                }
                placeholder='0.00'
                type='number'
                step='0.0001'
            ...
            <div className='space-y-2'>
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewForm({ ...newForm, cacheRead: e.target.value })
                }
                placeholder='0.00'
                type='number'
                step='0.0001'
            ...
            <div className='space-y-2'>
              ...
              <Input
                ...
                onChange={(e) =>
                  setNewForm({ ...newForm, cacheWrite: e.target.value })
                }
                placeholder='0.00'
                type='number'
                step='0.0001'
