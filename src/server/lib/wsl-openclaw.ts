...
let distributionCache: { expiresAt: number; value: WslDistribution[] } | null = null;
let defaultOpenClawPathCache: {
  expiresAt: number;
  value: ResolvedOpenClawPath | null;
} | null = null;
const homePathCache = new Map<string, { expiresAt: number; value: string | null }>();
...
function normalizeLinuxPath(value: string, distro: string): {
  linuxPath?: string;
  error?: string;
} {
...
export function getWslAvailability(distroName?: string): WslAvailability {
  if (process.platform !== 'win32') {
    return {
      available: false,
      distributions: [],
      error: 'WSL2 access is only required when Clawalytics is running on Windows.',
    };
  }
  ...
  if (distributions.length === 0) {
    return {
      available: false,
      distributions,
      error: 'No WSL distributions were found. Install WSL2 and an OpenClaw distribution first.',
    };
  }

  const distro = distroName
    ? distributions.find((candidate) => candidate.name === distroName)
    : distributions.find((candidate) => candidate.isDefault) ?? distributions[0];
    ...
    if (explicitWsl || isLikelyLinuxPath(rawPath)) {
      ...
      if (!distro) {
        return {
          ...
          source: 'wsl-linux',
          isWsl: true,
          error: 'No WSL distribution is available. Configure a WSL2 distribution name or install WSL2.',
          warnings: [],
        };
      }
...
export function detectDefaultWslOpenClawPath(): ResolvedOpenClawPath | null {
  ...
  if (defaultOpenClawPathCache && defaultOpenClawPathCache.expiresAt > Date.now()) {
